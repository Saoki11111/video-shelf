...
カルーセルと新着カードのサムネイルから動画詳細へ移動する。一覧への導線はヘッダーナビに集約する。
...
- `driveFileId` は元動画のダウンロード導線にのみ使用する。
...
3. 公開URLが `206 Partial Content`、`Content-Type: video/mp4`、`Accept-Ranges: bytes` を返すことを確認する。
