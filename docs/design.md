...
更新は年2〜3回とし、管理画面とログインは持たない。動画台帳はJSONで管理し、再生数だけをD1へ保存する。
...
flowchart LR
    ...
    Viewer -->|10秒以上再生| Function[Pages Functions<br>再生数API]
    Function -->|再生数を記録・取得| D1[D1<br>動画別再生数]
```
...
| Pages Functions | 再生数の登録・取得API |
| Cloudflare D1 | 動画別の再生数を保存 |
| Google Drive | 元動画の保管 |
| `videos.json` | 動画情報、新着日、場所タグを管理 |

Google Driveのiframe / preview埋め込みは使用しない。動画台帳はDBへ移さず、再生数だけをD1へ保存する。
...
フレームワーク、外部パッケージ、Webフォントは使用しない。サーバー処理は再生数APIだけに限定する。
...
動画を10秒以上再生した時点で再生数を記録する。同じブラウザ・同じ動画からの記録は24時間に1回とし、一覧の人気順に使用する。いいねは実装しない。
