...
```mermaid
flowchart LR
    Owner[管理者] -->|元動画を保管| Drive[Google Drive<br>保管・ダウンロード元]
...
[
  {
    ...
    "downloadUrl": "https://PUBLIC_BUCKET_URL/downloads/202603-hongkong.mp4",
...
- `downloadUrl` はR2上で `Content-Disposition: attachment` を設定したURLとする。
...
3. 同じMP4をR2の `downloads/` 以下へアップロードし、Content-Typeを `video/mp4`、Content-Dispositionを `attachment` にする。
4. 再生用URLが `206 Partial Content`、`Content-Type: video/mp4`、`Accept-Ranges: bytes` を返すことを確認する。
5. ダウンロード用URLが `Content-Disposition: attachment` を返すことを確認する。
6. サムネイルを `images/thumbnails/` に追加する。
7. `data/videos.json` にメタ情報、`videoUrl`、`downloadUrl`、`driveFileId` を追加する。
8. `./scripts/check.sh` とローカル表示を確認する。
9. 作業ブランチをpushし、PR・CIを経由してmergeする。
