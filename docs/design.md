...
2. 新着3本（PCは3列、SPは循環カルーセル）

カルーセルと新着カードのサムネイルから動画詳細へ移動する。新着見出し横のリンクとヘッダーナビから一覧へ移動できる。
...
- ヘッダーと「← 一覧へ」から移動
- 現在の動画を除くランダム3本を「次に見る」として常時表示。SPでは左右矢印とスワイプで循環し、再生終了時にその位置まで移動
...
| 機能 | 判定方法 |
| --- | --- |
| 新着 | `addedAt`、未設定の場合は`publishedAt`の降順 |
| Pickupカルーセル | ページ表示時に全動画から重複なしでランダム3本。5秒ごとの自動切り替えとスマホの左右スワイプに対応 |
| 新着カルーセル | SPでは新着3本を左右矢印とスワイプで循環。PCでは3本を同時表示 |
| タグ絞り込み | 複数の場所タグをOR条件で選択。「すべて」で解除 |
...
- サムネイルは表示のチラつきを避けるため、サイズを明示して優先読み込み・同期デコードする。
- 動画本体は詳細画面だけで読み込む。
- TOPで動画を自動再生しない。
